...
export const BusinessSelector: React.FC = () => {
  const { selectedBusiness, selectedRadius, setSelectedBusiness, setSelectedRadius } = useMapStore();
  const [selected, setSelected] = useState<BusinessCategory | null>(selectedBusiness);
  ...
  return (
    <div className="space-y-6">
      ...
      {selected && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Service Radius: {selectedRadius} km
          </label>
          <input
            type="range"
